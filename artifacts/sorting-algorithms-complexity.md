...
- JS's `Array.prototype.sort()` is TimSort in V8: O(n log n) worst, O(n) on sorted input, stable since ES2019. With no comparator it sorts as **strings** — `[10, 9, 1].sort()` gives `[1, 10, 9]`. Pass `(a, b) => a - b` for numbers.
